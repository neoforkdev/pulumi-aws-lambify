...
import type {
  ...
  ApiMethod,
  OpenApiSpec,
} from '../../model/type/domain/api-tree';
...
export class ApiParser extends Parser<string, ParsedApi> {
  ...
  private async parseApiRoutes(apiDirectory: string): Promise<ApiRoute[]> {
    ...
    return Promise.all(discoveredRoutes.map((route) => this.parseRoute(route)));
  }
  ...
  private async parseRoute(
    ...
    return {
      ...
      methods: methods.filter(
        (method): method is ApiMethod => method !== undefined,
      ),
    };
  }
  ...
  private async parseMethod(
    ...
    const filePaths = new MethodFilePaths(
      methodDir.directory,
      RequirementsFile[extension],
    );
    ...
    await validateFile(
      ...
      true,
      () =>
        new MissingConfigFileError(
          filePaths.configFile,
          `${route} ${methodDir.method.toUpperCase()}`,
        ),
    );
  ...
  private async parseMethodOpenApi(
    openApiFile: string,
